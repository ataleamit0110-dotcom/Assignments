...
def marvellousPredictor():
    ...
    # rSquare = Summation of (yp - y_bar)Square / summation of (y- y_bar)square // where yp is mx + c
    rSquare = yPredicted/Y_actual
    print(" R square :", rSquare)

    r2 = r2_score(Y, ypList)
    print("Actual values : Y ", Y)
    print("Predicted values : YP ", ypList)
    print("R Square value ", r2) # 0.307
    # Calculate MSE
    mse = 0
    for i in range(n):
        mse = mse + (Y[i] - ypList[i])**2
